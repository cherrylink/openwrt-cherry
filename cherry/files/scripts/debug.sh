...
enabled=`uci get cherry.config.enabled`

if [ "$enabled" == "0" ]; then
	uci set cherry.config.enabled=1
	uci commit cherry
	/etc/init.d/cherry restart
fi
...
`
if [ -x "/bin/opkg" ]; then
	opkg list-installed "cherry"
	opkg list-installed "luci-app-cherry"
elif [ -x "/usr/bin/apk" ]; then
	apk list -I "cherry"
	apk list -I "luci-app-cherry"
fi
`
\`\`\`
## config
...
const config = uci.get_all("cherry");
const result = {};
...
function desensitize_profile() {
	let profile = {};
	const process = popen("yq -p yaml -o json /etc/cherry/run/config.yaml");
	if (process) {
...
TPROXY: 
`
ip route list table "$(uci get cherry.routing.tproxy_route_table)"
`

TUN: 
`
ip route list table "$(uci get cherry.routing.tun_route_table)"
`
\`\`\`
## ip6 rule
\`\`\`
...
TPROXY: 
`
ip -6 route list table "$(uci get cherry.routing.tproxy_route_table)"
`

TUN: 
`
ip -6 route list table "$(uci get cherry.routing.tun_route_table)"
`
\`\`\`
## nftables
\`\`\`
`
nft list table inet cherry
`
\`\`\`
## service
\`\`\`json
`
/etc/init.d/cherry info
`
\`\`\`
"

if [ "$enabled" == "0" ]; then
	uci set cherry.config.enabled=0
	uci commit cherry
	/etc/init.d/cherry restart
fi
