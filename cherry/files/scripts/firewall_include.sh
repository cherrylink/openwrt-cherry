...
if [ "$enabled" = 1 ] && [ "$core_only" = 0 ] && [ "$proxy_enabled" = 1 ]; then
	if [ "$tcp_mode" = "tun" ] || [ "$udp_mode" = "tun" ]; then
		...
		nft insert rule inet fw4 input iifname "$tun_device" counter accept comment "cherry"
		nft insert rule inet fw4 forward oifname "$tun_device" counter accept comment "cherry"
		nft insert rule inet fw4 forward iifname "$tun_device" counter accept comment "cherry"
	fi
fi
